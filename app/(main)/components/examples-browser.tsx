...
/** One sidebar item: a category, and the examples that fell into it. */
type Group = {
  slug: string
  name: string
  icon: IconSvgElement
 ...
 * never a category's, so the two can't collide in the rail.
 */
 ...
 * — `/tokens` passes the token ones, `/elements` the rest — so a new demo needs
 * an item in `registry/_examples-registry.ts`, its components in
 * `registry/_demos.ts`, and nothing here.
 *
    ...
    <div className="mt-10">
      ...
      <div className="mt-6 flex flex-col gap-6 md:flex-row md:gap-10">
        {/* The rail is a nav of filters, not of destinations, so the items are
            ...
            whose role promises arrow-key navigation between them. */}
        <nav
          aria-label="Categories"
          className="-mx-4 px-4 md:mx-0 md:w-40 md:shrink-0 md:px-0"
        >
          ...
          <div className="flex gap-1 overflow-x-auto pb-1 md:sticky md:top-15 md:flex-col md:overflow-visible md:pb-0">
            {groups.map((item) => {
